...
BLXAPI blxFreeList* blxFreeList_Create(void* memory, u64 size);
...
BLXAPI void* blxFreeList_GetMem(blxFreeList* freeList, u64 size);
...
BLXAPI u64 blxFreeList_GetFreeMemCount(blxFreeList* freeList);

/// @brief Gets the total number of memory currently allocated from the free list.
/// @param freeList Pointer to the free list.
/// @return Number of memory blocks.
BLXAPI u64 blxFreeList_GetMemCount(blxFreeList* freeList);
...
BLXAPI u64 blxFreeList_GetInternalSize();
...
BLXAPI u64 blxFreeList_GetMemSize(void* mem);