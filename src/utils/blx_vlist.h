...
///The initial size of our list.
#define BLX_LIST_INITIAL_SIZE 2
...
#define blxInitList(type) (type*)_blxInitList(sizeof(type), BLX_LIST_INITIAL_SIZE)
#define blxInitListWithSize(type, size) (type*)_blxInitList(sizeof(type), size)
#define _BLX_HEADER_SIZE sizeof(unsigned long) * 3
#define _BLX_GET_HEADER(list, headerElement) *((unsigned long*)list - _BLX_HEADER_SIZE + headerElement)
#define _BLX_SET_HEADER(list, headerElement, value) *((unsigned long*)list - _BLX_HEADER_SIZE + headerElement) = (value)
...
{
    unsigned long* list = (unsigned long*)malloc(_BLX_HEADER_SIZE + intitialSize * elementSize);
    list[_BLX_MAX_LENGTH_I] = intitialSize;
    list[_BLX_COUNT_I] = 0;
    list[_BLX_ELEMENT_SIZE_I] = elementSize;
...
static void* _blxAddToList(void* list, void* element)
{
    ...
    if (count >= maxLength) {
        ...
        memcpy((char*)newList, (char*)list, (count) * (elementSize));
        _BLX_SET_HEADER(newList, _BLX_COUNT_I, count);
        free(((unsigned long*)list - _BLX_HEADER_SIZE));
        list = newList;
    }
